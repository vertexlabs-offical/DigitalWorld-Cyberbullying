...
/* ─── Katana transition overlay ──────────────────────────────── */
function KatanaTransition() {
  return (
    <div className="fixed inset-0 z-[200] pointer-events-none overflow-hidden">
      {/* Horizontal speed bursts at flash peak */}
      <div className="katana-speed-left" />
      <div className="katana-speed-right" />
      {/* Afterimage trails (rendered behind the blade) */}
      <div className="katana-trail-2" />
      <div className="katana-trail-1" />
      {/* Primary blade */}
      <div className="katana-blade" />
      {/* White flash at apex */}
      <div className="katana-flash" />
    </div>
  );
}
...
export default function App() {
  ...
  // Clear all pending timers on unmount to prevent state updates on dead trees
  useEffect(() => () => { timers.current.forEach(clearTimeout); }, []);

  /** Navigate with katana-slash transition */
  const navigate = useCallback((dest: Page) => {
    if (dest === page || slashing) return;
    setMenuOpen(false);
    nextPage.current = dest;

    // Clear any stale timers
    timers.current.forEach(clearTimeout);
    timers.current = [];

    // 1. Trigger blade + shake
    setSlashing(true);
    setShaking(true);
  ...
  return (
    /* screen-shake added transiently on nav */
    <div className={`relative w-full min-h-screen overflow-x-hidden bg-black font-inter${shaking ? ' screen-shake' : ''}`}>

      {/* ── Katana overlay (mounted only during transition) ─────── */}
      {slashing && <KatanaTransition />}
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-6 sm:px-10 lg:px-16 py-5 lg:py-7 backdrop-blur-sm border-b border-white/5">
        ...
        <button
          ...
          className="font-podium text-2xl sm:text-3xl font-bold uppercase tracking-wider text-white hover:text-cyan-300 transition-colors"
        >
          DIGITAL WORLD
        </button>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 sm:px-10 py-5">
          ...
          <button onClick={() => setMenuOpen(false)} className="text-white/70 hover:text-cyan-300 transition-colors" aria-label="Close menu">
            <X className="w-7 h-7" />
          </button>
        </div>
      ...
      {/* ── Page content ─────────────────────────────────────────── */}
      {/* key={page} forces remount → entrance animations replay */}
      <main key={page} className="relative z-10 page-reveal">
    ...
    <section className="min-h-screen flex flex-col justify-center px-6 sm:px-10 lg:px-16 pt-24 pb-12">
      ...
      <div className="flex items-center gap-2 mb-6 lg:mb-8 animate-fade-up">
        ...
        <span className="font-inter text-xs sm:text-sm text-white/70 tracking-[0.3em] uppercase">
          IGCSE Research Project · EPS Bhopal · 2026
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 sm:gap-6 mt-8 lg:mt-10 animate-fade-up-delay-3">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white/60 text-xs tracking-wider uppercase">Project 2026</p>
          </div>
        </div>
      </div>
    ...
    <section className="min-h-screen pt-28 pb-16 px-6 sm:px-10 lg:px-16">
      ...
      <div className="animate-fade-up-delay-1 mt-8 border border-purple-500/30 bg-purple-500/10 p-6 max-w-2xl">
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="font-podium text-xl text-white uppercase tracking-wider">EPS Bhopal · 2026</p>
            <p className="font-inter text-white/60 text-sm mt-1 leading-relaxed">
              ...
              programme at EPS Bhopal. This website is our 2026 research project focused on spreading awareness about cyberbullying in the digital world.
            </p>
          </div>
        </div>
      </div>
    ...
    <section className="min-h-screen pt-28 pb-16 px-6 sm:px-10 lg:px-16">
      ...
      <div className="animate-fade-up-delay-1 mt-6 flex items-start gap-3 border border-red-500/40 bg-red-500/10 p-4 max-w-3xl">
        ...
        <p className="font-inter text-white/80 text-sm leading-relaxed">
          <strong className="text-red-300">Important:</strong> If you or someone you know is in immediate danger, contact your local emergency services or a trusted adult immediately.
        </p>
      </div>
...
function TypesTab() {
  const types = [
    { name: 'Harassment',       color: 'text-red-400',    border: 'border-red-500/30',    bg: 'bg-red-500/5',    desc: 'Sending offensive, rude, or insulting messages repeatedly to an individual or group. This is the most common form of cyberbullying (Kowalski et al., 2014).' },
    { name: 'Cyberstalking',    color: 'text-orange-400', border: 'border-orange-500/30', bg: 'bg-orange-500/5', desc: 'Repeated, intense harassment that includes threats and causes significant fear in the target. Often escalates to physical harm (Bauman, 2011).' },
    { name: 'Exclusion',        color: 'text-yellow-400', border: 'border-yellow-500/30', bg: 'bg-yellow-500/5', desc: 'Intentionally leaving someone out of an online group, game, or social circle. Particularly harmful for adolescents (Boyd, 2014).' },
    { name: 'Outing & Trickery',color: 'text-purple-400', border: 'border-purple-500/30', bg: 'bg-purple-500/5', desc: 'Sharing private or embarrassing information online without consent, sometimes after gaining trust and extracting secrets (Willard, 2007).' },
    { name: 'Impersonation',    color: 'text-cyan-400',   border: 'border-cyan-500/30',   bg: 'bg-cyan-500/5',   desc: 'Creating fake accounts or hacking into someone\'s accounts to send harmful messages and damage their reputation (Hinduja &amp; Patchin, 2019).' },
    { name: 'Flaming',          color: 'text-pink-400',   border: 'border-pink-500/30',   bg: 'bg-pink-500/5',   desc: 'Online fights using electronic messages with angry, vulgar language. Often occurs in public forums and comment sections (Willard, 2007).' },
  ];
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {types.map((t) => (
        <div key={t.name} className={`border ${t.border} ${t.bg} p-5 transition-all duration-300 hover:scale-[1.02]`}>
          ...
          <p className="font-inter text-white/60 text-xs leading-relaxed" dangerouslySetInnerHTML={{ __html: t.desc }} />
        </div>
      ))}
    </div>
  );
...
function EffectsTab() {
  return (
    <div className="space-y-6">
      ...
      {[
        { category: 'Mental Health Effects', color: 'text-red-300',    effects: ['Increased rates of depression and anxiety (Kowalski et al., 2014)', 'Post-traumatic stress symptoms (Ybarra &amp; Mitchell, 2004)', 'Low self-esteem and body image issues (Fardouly &amp; Vartanian, 2015)', 'Suicidal ideation — victims are 2× more likely to attempt suicide (Hinduja &amp; Patchin, 2010)'] },
        { category: 'Academic Effects',      color: 'text-yellow-300', effects: ['Declining grades and motivation (Beran &amp; Li, 2007)', 'Increased school absenteeism', 'Difficulty concentrating in class', 'School avoidance and dropout risk (Kowalski &amp; Limber, 2013)'] },
        { category: 'Social Effects',        color: 'text-cyan-300',   effects: ['Social withdrawal and isolation (Boyd, 2014)', 'Damaged friendships and peer relationships', 'Fear of using the internet or technology', 'Loss of trust in digital communication (Hinduja &amp; Patchin, 2019)'] },
        { category: 'Physical Effects',      color: 'text-purple-300', effects: ['Sleep disturbances and insomnia', 'Headaches and stomach aches', 'Loss of appetite', 'Increased substance use as a coping mechanism (Ybarra &amp; Mitchell, 2004)'] },
      ].map((section) => (
        <div key={section.category}>
          <h3 className={`font-podium uppercase tracking-wider text-lg ${section.color} mb-3`}>{section.category}</h3>
...
function ActionTab() {
  const steps = [
    { num: '01', title: "Don't Respond or Retaliate", color: 'text-cyan-300',   border: 'border-cyan-400/30',   desc: "Responding often escalates the situation. Retaliation can make you look like the bully. Step away from the screen and take a breath (Hinduja &amp; Patchin, 2019)." },
    { num: '02', title: 'Document Everything',         color: 'text-purple-300', border: 'border-purple-400/30', desc: 'Take screenshots of all bullying messages, posts, and comments with timestamps. This evidence is crucial if you need to report to authorities or school officials (Willard, 2007).' },
    { num: '03', title: 'Block and Report',            color: 'text-yellow-300', border: 'border-yellow-400/30', desc: 'Use platform tools to block the bully and report the behaviour. All major social media platforms have reporting mechanisms (UNICEF, 2020).' },
    { num: '04', title: 'Tell a Trusted Adult',        color: 'text-green-300',  border: 'border-green-400/30',  desc: 'Talk to a parent, guardian, teacher, or school counsellor. Adults can help mediate and take formal action. Research shows most victims do NOT tell adults — this must change (Pew Research Center, 2022).' },
    { num: '05', title: 'Seek Professional Support',   color: 'text-pink-300',   border: 'border-pink-400/30',   desc: 'Cyberbullying causes real psychological harm. Speaking to a mental health professional or counsellor is a sign of strength, not weakness (Kowalski et al., 2014).' },
    { num: '06', title: 'Know Your Rights',            color: 'text-red-300',    border: 'border-red-400/30',    desc: 'Many countries have laws against cyberbullying. In India, cyberbullying can be reported under the IT Act 2000 (Sections 66A, 67) and the Indian Penal Code (Sections 499, 507).' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="border border-white/10 bg-white/5 p-6 mt-8">
        ...
        <ul className="space-y-2 font-inter text-white/50 text-xs leading-relaxed">
          ...
          <li>Willard, N. E. (2007). <em>Cyberbullying and cyberthreats.</em> Research Press.</li>
          <li>Ybarra, M. L., &amp; Mitchell, K. J. (2004). Online aggressor/targets, aggressors, and targets. <em>Journal of Child Psychology and Psychiatry, 45</em>(7), 1308–1316.</li>
        </ul>
      </div>
    </div>
  );
